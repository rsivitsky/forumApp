...
CREATE TABLE IF NOT EXISTS `forum`.`message` (
  ...
  `topic_user_id` INT NOT NULL,
  `user_id` INT NOT NULL,
  PRIMARY KEY (`id`, `topic_id`, `topic_user_id`, `user_id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC),
  INDEX `fk_message_topic1_idx` (`topic_id` ASC, `topic_user_id` ASC),
  INDEX `fk_message_user1_idx` (`user_id` ASC),
  CONSTRAINT `fk_message_topic1`
    FOREIGN KEY (`topic_id` , `topic_user_id`)
    REFERENCES `forum`.`topic` (`id` , `user_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
CREATE TABLE IF NOT EXISTS `forum`.`attache` (
  ...
  `message_topic_user_id` INT NOT NULL,
  `message_user_id` INT NOT NULL,
  PRIMARY KEY (`id`, `message_id`, `message_topic_id`, `message_topic_user_id`, `message_user_id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC),
  INDEX `fk_atache_message1_idx` (`message_id` ASC, `message_topic_id` ASC, `message_topic_user_id` ASC, `message_user_id` ASC),
  CONSTRAINT `fk_atache_message1`
    FOREIGN KEY (`message_id` , `message_topic_id` , `message_topic_user_id` , `message_user_id`)
    REFERENCES `forum`.`message` (`id` , `topic_id` , `topic_user_id` , `user_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
